...
std::vector<std::shared_ptr<Tensor>> CausalSelfAttention::Forward(const std::vector<std::shared_ptr<Tensor>> &x) {
    ...
    if (config_.use_flash_attn) {
        // Flash Attention path: expects (B, T, H, D) layout
        // q, k, v are already in (B, T, H_local, D) format

        // Call Flash Attention
        y = nn::function::ScaledDotProductAttention(
            q, k, v,
            /*attn_mask=*/nullptr,  // Use mask parameter if provided
            /*dropout_p=*/0.0f,
            /*is_causal=*/mask != nullptr ? false : true,  // If mask provided, don't use causal
